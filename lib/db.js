...
export function orSelectPromisified(conditions, collection) {
  ...
  return new Promise ( (resolve, reject) => {
    ...
    const conditionsTasks = conditions.map( (cond) => {
    ...
    async.series(conditionsTasks, (err, values) => {
      ...
      const merged = lodash.reduce(values, (acc, v) => {

        const objs = v.map( (e) => toOutside(e.data$()) )
        return acc.concat(objs)

      }, [])

      resolve(lodash.uniqBy(merged, '_id'))
    })
