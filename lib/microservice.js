 ...
 * @function createDiscoveryFunction
 * @param {String} namespace kubernetes namespace where to find the pods
 * @returns {Function}
 */
export function createDiscoveryFunction(namespace) {
  return async function serviceDiscovery(seneca, options, bases, next) {
    ...
    try {
      ...
      const servicesIps = items.reduce((accum, {
        metadata: {name},
        status: {podIP},
        spec: {containers}
      }) => {
        if (name.match(/grm-seneca/i)) {
          const podPort = containers[0].ports.find(({name}) => name === 'http').containerPort
          const ip = `${podIP}:${podPort}`
